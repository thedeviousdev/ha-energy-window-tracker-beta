...
class EnergyWindowConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _is_runtime_setup_mode(self) -> bool:
        """Return True when setup already created an entry in this flow."""
        return (
            (not self._pending_sources or not isinstance(self._pending_sources[0], dict))
            and bool(self._pending_setup_entry_id)
        )

    def _get_runtime_setup_entry(self) -> config_entries.ConfigEntry | None:
        """Return the created setup entry when present."""
        if not self._pending_setup_entry_id:
            return None
        return self.hass.config_entries.async_get_entry(self._pending_setup_entry_id)

    def _get_active_windows_for_configure(self) -> list[dict[str, Any]]:
        """Get windows from pending source or from the runtime-created entry."""
    ...
    async def _set_active_windows_for_configure(
        self, windows: list[dict[str, Any]]
    ) -> None:
        """Persist windows to pending source or to the runtime-created entry."""
        if self._is_runtime_setup_mode():
            existing_entry = self._get_runtime_setup_entry()
            if existing_entry is not None:
                self._setup_windows = windows
    ...
    async def _update_runtime_setup_windows_group(
        self,
        *,
        ...
        if existing_entry is None:
            return
        existing_windows_raw = existing_entry.data.get(CONF_WINDOWS) or []
        if not isinstance(existing_windows_raw, list):
            existing_windows_raw = []

        updated_windows: list[dict[str, Any]] = []
        normalized_target = (target_window_name or "").strip()
        for window in existing_windows_raw:
            if not isinstance(window, dict):
                continue
            current_name = (window.get(CONF_WINDOW_NAME) or "").strip()
        ...
        self._setup_windows = updated_windows
        self.hass.config_entries.async_update_entry(
            existing_entry,
            data={CONF_WINDOWS: updated_windows},
        )

    async def _delete_runtime_setup_window_group(self, *, target_window_name: str) -> None:
        """Delete a window group in runtime setup entry while preserving other rows."""
        existing_entry = self._get_runtime_setup_entry()
        if existing_entry is None:
            return
        existing_windows_raw = existing_entry.data.get(CONF_WINDOWS) or []
        ...
        updated_windows = [
            ...
            and (window.get(CONF_WINDOW_NAME) or "").strip() != normalized_target
        ]
        self._setup_windows = updated_windows
        self.hass.config_entries.async_update_entry(
            existing_entry,
    ...
    async def async_step_window_entities_confirm(
        ...
        """Confirmation after adding entities; continue to configure menu on Finish."""
    ...
    async def async_step_add_window(
        ...
        if user_input is not None and "start_1" in user_input:
            ...
            if _window_name_already_exists(
                self._pending_sources[0].get(CONF_WINDOWS, []), w_name or ""
            ):
                ranges_for_form = [{"start": s, "end": e} for s, e in ranges]
                schema = _build_single_window_multi_range_schema(
                    labels,
                    None,
                    w_name or "",
                ...
                return self.async_show_form(
                    step_id="add_window",
                    data_schema=schema,
                    errors={"base": "duplicate_window_name"},
                )
            if user_input.get("add_another"):
                _MAIN_LOGGER.debug(
    ...
    async def async_step_edit_window(
        ...
        if not edit_name:
            return await self.async_step_configure_menu(None)
        form_title = _configure_title(edit_name)
        def _show_edit_form(
            data_schema: vol.Schema, errors: dict[str, str] | None = None
        ) -> config_entries.FlowResult:
            result = self.async_show_form(
                step_id="edit_window", data_schema=data_schema, errors=errors or {}
            )
...
def _get_sources_from_entry(entry: config_entries.ConfigEntry) -> list[dict[str, Any]]:
    ...
    Canonical format is window-first: entry data/options contain `windows`
    (CONF_WINDOWS), where each row contains `entities` (CONF_ENTITIES) and
    `ranges` (CONF_RANGES). This helper groups those rows by source entity for
    options-flow source management.
    """
...
def _sources_to_windows(sources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Convert source-grouped rows back into canonical windows-based rows."""
    ...
    for source in sources:
        ...
        for window in source_windows:
            if not isinstance(window, dict):
                continue
            range_start = _time_to_str(window.get(CONF_WINDOW_START))
            range_end = _time_to_str(window.get(CONF_WINDOW_END))
...
def _window_name_already_exists(
    windows: list[dict[str, Any]],
    candidate_name: str,
    *,
    exclude_raw_name: str | None = None,
) -> bool:
    """Return True if candidate_name conflicts with another existing window name."""
    candidate = (candidate_name or "").strip()
    if not candidate:
        return False
    excluded = (exclude_raw_name or "").strip()
    for window in windows:
        raw_name = (window.get(CONF_WINDOW_NAME) or "").strip()
        if excluded and raw_name == excluded:
            continue
...
class EnergyWindowOptionsFlow(config_entries.OptionsFlow):
    ...
    def __init__(self, config_entry: config_entries.ConfigEntry) -> None:
        ...
        self._pending_add_cost: float = 0.0
        self._pending_add_ranges: list[dict[str, str]] = []
        self._source_entity_to_edit: str | None = None
    ...
    def _get_current_source(self) -> dict[str, Any]:
        ...
        if self._source_entity_to_edit:
            selected_entity = self._source_entity_to_edit.strip()
            for source in sources:
                if not isinstance(source, dict):
                    continue
                source_entity = str(source.get(CONF_SOURCE_ENTITY) or "").strip()
                if source_entity == selected_entity:
                    return source
    ...
    def _async_create_options_entry(
        ...
        windows = options.get(CONF_WINDOWS) if isinstance(options, dict) else None
    ...
    async def async_step_source_entity(
        ...
        if user_input is not None and CONF_ENTITIES in user_input:
            ...
            removed_entities = set(available_source_entities) - set(requested_entities)
            # Treat entity changes as replacement, but only remove registry entities
            # associated with source entities that were explicitly deselected.
            if removed_entities:
                removed_source_slugs = {
                    source_slug_from_entity_id(entity_id)
